    ...
    public class PaymentProcessor : IPaymentProcessor
    {
        ...
        private void ExecutePayments(IList<IWorkerBalance> workerBalances)
        {
            ...
            try
            {
                ...
                var txid = _daemonClient.SendMany(_poolAccount, payments); // send the payments
        ...
        private void QueryBlock(ref IPersistedBlock block)
        {
			...
			// make sure we have a valid reference to genTx
			if (genTx == null)
			{
				block.Status = BlockStatus.Orphaned;
				return;
        ...
        private IList<IPaymentRound> GetPaymentRounds(IEnumerable<IPersistedBlock> blocks)
        {
            ...
            {
                var round = new PaymentRound(block,_rewardsConfig);
                rounds.Add(round);
            }
